    ...
    <div className="min-h-screen pb-24 bg-slate-50/70">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 space-y-6">
        ...
        {loading ? (
          ...
          <div className="bg-white p-8 sm:p-12 text-center rounded-2xl border border-dashed border-slate-300 shadow-2xs">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-sky-50 to-indigo-50 text-sky-600 flex items-center justify-center mx-auto mb-4 border border-sky-100">
              <Plane className="w-7 h-7 -rotate-45" />
            </div>
            <h3 className="text-base font-bold text-slate-900 mb-1.5">
              {routes.length === 0
                ? !user
                  ? "Monitore seus voos favoritos"
                  : t.routes.emptyTitle
                : t.common.noResults}
            </h3>
            <p className="text-xs text-slate-600 max-w-md mx-auto mb-6 font-medium leading-relaxed">
              {routes.length === 0
                ? !user
                  ? "Adicione rotas para acompanhar tarifas aéreas em tempo real e receber alertas de preços no seu e-mail."
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                ...
                {!user && (
                  <button
                    ...
                    className="w-full sm:w-auto px-4 py-2.5 rounded-xl text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 border border-slate-200/80 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                  >
                    <LogIn className="w-3.5 h-3.5 text-sky-600" />
                    <span>{t.auth.loginPromptButton}</span>
                  </button>
                )}
              </div>
            ) : (
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    className={`p-3.5 sm:px-6 sm:py-4 bg-slate-50/70 hover:bg-slate-100/70 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-3.5 cursor-pointer select-none transition-colors ${
                      isCollapsed ? "rounded-2xl" : "rounded-t-2xl border-b border-slate-200/80"
                    }`}
                  >
                    <div className="flex items-start sm:items-center gap-3 sm:gap-3.5 min-w-0">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white border border-slate-200/80 shadow-2xs flex items-center justify-center text-slate-500 hover:text-slate-800 shrink-0 transition-transform mt-0.5 sm:mt-0">
                        {isCollapsed ? (
                      ...
                      <div className="flex flex-col gap-0.5 min-w-0">
                        <div className="flex items-center gap-1.5 sm:gap-2 font-black text-slate-900 tracking-tight text-sm sm:text-lg flex-wrap">
                          <span>{group.origin}</span>
                          <ArrowRight className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-sky-600 stroke-[2.5] shrink-0" />
                          <span>{group.destination}</span>

                          <span className="px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-semibold bg-slate-200/80 text-slate-700 ml-0.5 sm:ml-1 shrink-0">
                            {group.routes.length}{" "}
                            {locale === "en"
                    ...
                    {group.lowestPrice !== null && group.lowestPriceRoute ? (
                      <div className="flex items-center justify-between sm:flex-col sm:items-end gap-1 shrink-0 pt-2.5 sm:pt-0 border-t border-slate-200/70 sm:border-0">
                        {/* Mobile: Label + Badge agrupados à esquerda | Desktop: STARTING FROM acima */}
